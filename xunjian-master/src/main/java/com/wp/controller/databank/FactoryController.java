 ...
 */
    ...
    public class FactoryController extends BaseController {
        String menuUrl = "factory/list.do"; //菜单地址(权限用)
        ...
        public ModelAndView edit() throws Exception{
            logBefore(logger, "修改工厂信息");
            if(!Jurisdiction.buttonJurisdiction(menuUrl, "edit")){return null;} //校验权限
            ModelAndView mv = this.getModelAndView();
            PageData pd = new PageData();
            pd = this.getPageData();
            factoryService.edit(pd);
            mv.addObject("msg","success");
            mv.setViewName("save_result");
            return mv;
        ...
        public ModelAndView list(Page page){
            ...
            try{
                ...
                page.setPd(pd);
                List<PageData> varList = factoryService.list(page);	//列出${objectName}列表
                mv.setViewName("databank/factory");
        ...
        public ModelAndView goAdd(){
            logBefore(logger, "去新增工厂页面");
            ModelAndView mv = this.getModelAndView();
            PageData pd = new PageData();
            pd = this.getPageData();
            try {
                mv.setViewName("databank/factory_edit");
        ...
        public ModelAndView goEdit(){
            logBefore(logger, "去修改工厂页面");
            ModelAndView mv = this.getModelAndView();
            PageData pd = new PageData();
            pd = this.getPageData();
            try {
                pd = factoryService.findById(pd);	//根据ID读取
                mv.setViewName("databank/factory_edit");
        ...
        public Object deleteAll() {
            ...
            try {
                ...
                if(null != DATA_IDS && !"".equals(DATA_IDS)){
                    ...
                    factoryService.deleteAll(ArrayDATA_IDS);
                    pd.put("msg", "ok");
                }else{
                    pd.put("msg", "no");
                }
        ...
        public ModelAndView exportExcel(){
            ...
            try{
                ...
                titles.add("工厂名称");    //${var_index+1}
                titles.add("负责人");
                titles.add("手机号");
                //titles.add("职位");
               // titles.add("创建时间");
                dataMap.put("titles", titles);
                List<PageData> varOList = factoryService.listAll(pd);
                List<PageData> varList = new ArrayList<PageData>();
                for (int i = 0; i < varOList.size(); i++) {
                    PageData vpd = new PageData();
                    vpd.put("var1", varOList.get(i).getString("factory"));
        ...
        public ModelAndView readExcel(
            ...
            if (null != file && !file.isEmpty()) {
                ...
                for(PageData pds : listPd) {
                    ...
                    pd.put("factory", pds.getString("var0"));
