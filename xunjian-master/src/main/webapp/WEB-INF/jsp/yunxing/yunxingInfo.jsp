            ...
            <form action="yunxing/list.do" method="post" name="Form" id="Form">
               ...
               <%--搜索--%>
                ...
                <table id="table_report" class="table table-striped table-bordered table-hover">
                    ...
                    <!-- 开始循环 -->
                    <c:choose>
                        <c:when test="${not empty yunxingList}">
                            <%--<c:if test="${QX.cha == 1 }">--%>
                            <c:forEach items="${yunxingList}" var="var" varStatus="vs">
                                <tr>
                                    <td class='center' style="width: 20px;">${var.Date}</td>
                                    <td class='center' style="width: 20px;">${var.zhiBanTime}</td>
                                    <td style="width: 60px;" class="center">${var.paiFangZhiShu}</td>
                                    <td style="width: 139px;" class="center">${var.riPaiFangDunShu}</td>
...
<script type="text/javascript">
    ...
    //导出excel
    function toExcel(){
        window.location.href='<%=basePath%>yunxing/excel.do';
    }
