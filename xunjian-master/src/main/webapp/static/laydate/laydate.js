...
!function (a) {
    ...
    a.laydate = function (b) {
        ...
        var b = c.query, d = "";
        c.each(new Array(8), function (b) {
            d += 4 === b ? "<li " + (parseInt(h.year.value) === a ? 'class="' + h[2] + '"' : "") + ' y="' + a + '">' + a + "年</li>" : '<li y="' + (a - 4 + b) + '">' + (a - 4 + b) + "年</li>"
        ...
        c.addClass(d.body, "laydate_body"), h.tds = b("#laydate_table td"), h.mms = b("#laydate_ms span"), h.year = b("#laydate_y"), h.month = b("#laydate_m"), c.each(b(e.box + " .laydate_ym"), function (a, b) {
            ...
            c.on(b, "click", function (a) {
                c.each(
                    $("#laydate_table td"),function(i,n){
                        if(c.hasClass(n, h[2])){
                            c.removeClass(n, h[2]);
                        }
                    }
                );
                c.addClass(this,"laydate_click");
                c.hasClass(this, h[1]) || (c.stopmp(a), c.creation1([0 | this.getAttribute("y"), 0 | this.getAttribute("m"), 0 | this.getAttribute("d")]))
            })
            ...
            //var std = $(".laydate_click");
            //var y = $(std[1]).attr("y");
            //var m = $(std[1]).attr("m");
            //var d = $(std[1]).attr("d");

            var std = $(".laydate_table .laydate_click").last();
            if(std.length != 0){
                var y = $(std).attr("y");
                var m = $(std).attr("m");
                var d = $(std).attr("d");
                c.valid && c.creation([y, m, d])
            }
